...
function buildObject(){
  ...
  const d=data(), c=new THREE.Color(d.color);
  const outerGeo=new THREE.IcosahedronGeometry(1.65,2);
  const outerMat=new THREE.MeshStandardMaterial({color:c,roughness:.18,metalness:.75,transparent:true,opacity:.72,emissive:c,emissiveIntensity:.28});
  const outer=new THREE.Mesh(outerGeo,outerMat);
  const coreGeo=new THREE.IcosahedronGeometry(.9,1);
  const coreMat=new THREE.MeshStandardMaterial({color:0xffffff,roughness:.1,metalness:.8,emissive:c,emissiveIntensity:2.5});
  const core=new THREE.Mesh(coreGeo,coreMat);
  group.add(outer,core);
  sceneState.outer=outer; sceneState.core=core;
  sceneState.coreLight.color.copy(c); sceneState.coreLight.intensity=12;
}
