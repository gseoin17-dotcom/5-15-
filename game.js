...
let playerName = localStorage.getItem("jion_player_name") || "";
...
function renderRanking(){
  ...
  if(document.activeElement!==input && input.value!==playerName) input.value=playerName;
  const name=playerName.trim();
  if(!name){
    list.innerHTML='<div class="ranking-empty">이름을 입력하면<br><b>실시간으로 랭킹이 생성됩니다.</b></div>';
    return;
  }
  const s1=state.seasonData[1], s2=state.seasonData[2];
  const bestSeason=s2.max_level>0 ? 2 : 1;
  const bestLevel=Math.max(s1.max_level||0,s2.max_level||0);
  const totalPoints=state.pointsEarnedTotal||0;
  list.innerHTML=`<div class="rank-card">
    <div class="rank-no">#1</div>
    <div class="rank-main"><div class="rank-name">${escapeHtml(name)}</div><div class="rank-sub">시즌 ${bestSeason} · 최고 ${bestLevel}단계 · 실시간 반영</div></div>
    <div class="rank-score"><b>${totalPoints.toLocaleString("ko-KR")}P</b><small>포인트</small></div>
  </div>`;
}
...
function resetGame(){
  if(!confirm("정말 모든 게임 데이터를 초기화할까요?\n이름과 강화 진행도도 함께 삭제됩니다.")) return;
  localStorage.removeItem("jion_smell_game_v3");
  localStorage.removeItem("jion_player_name");
  location.reload();
}
...
if(playerNameInput){
  ...
  playerNameInput.addEventListener("input",()=>{
    ...
    localStorage.setItem("jion_player_name",playerName);
    renderRanking();
  });
}
