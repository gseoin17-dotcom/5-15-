...
function makeLevelModel(level, color, season2){
  ...
  const emissive = c.clone();
  const mat = (opts={}) => new THREE.MeshStandardMaterial(Object.assign({
    color:c, roughness:.18, metalness:.72, emissive, emissiveIntensity:.55,
    transparent:true, opacity:.86
  }, opts));
  const glowMat = (opts={}) => new THREE.MeshStandardMaterial(Object.assign({
    color:0xffffff, roughness:.08, metalness:.85, emissive, emissiveIntensity:2.8,
    transparent:true, opacity:.95
  }, opts));
  const add = (mesh, x=0,y=0,z=0) => { mesh.position.set(x,y,z); g.add(mesh); return mesh; };
  const torus = (r,t,ry=0,rz=0) => { const m=add(new THREE.Mesh(new THREE.TorusGeometry(r,t,12,48),mat())); m.rotation.y=ry;m.rotation.z=rz;return m; };
  const crystal = (sx,sy,sz) => add(new THREE.Mesh(new THREE.OctahedronGeometry(1,0),glowMat({opacity:.9}))).scale.set(sx,sy,sz);
  const sphere = (r=1, seg=24) => add(new THREE.Mesh(new THREE.SphereGeometry(r,seg,seg),mat()));
  const ico = (r=1, detail=1) => add(new THREE.Mesh(new THREE.IcosahedronGeometry(r,detail),mat()));
  const box = (x,y,z) => add(new THREE.Mesh(new THREE.BoxGeometry(x,y,z),mat()));
  const cyl = (r=.7,h=1.4,rs=12) => add(new THREE.Mesh(new THREE.CylinderGeometry(r,r,h,rs),mat()));
  const cone = (r=.9,h=1.8,rs=16) => add(new THREE.Mesh(new THREE.ConeGeometry(r,h,rs),mat()));

  // 60 visibly different level silhouettes. Season 2 starts a second visual language.
  const k = level + (season2 ? 35 : 0);
  switch(k){
    case 0: sphere(.85); break;
    case 1: ico(1.25,1); break;
    case 2: box(1.8,1.0,1.2); box(.65,1.8,.65).position.y=.45; break;
    case 3: cyl(.9,1.9,10); torus(1.05,.10,Math.PI/2); break;
    case 4: cone(1.15,2.1,6); torus(.85,.10,0,Math.PI/2); break;
    case 5: crystal(1.0,1.55,1.0); torus(.75,.09,Math.PI/2); break;
    case 6: { for(let i=0;i<3;i++){let m=ico(.72,1);m.position.x=(i-1)*.9;m.position.y=Math.abs(i-1)*.25;} break; }
    case 7: { torus(1.15,.16,Math.PI/2); ico(.55,1); break; }
    case 8: { sphere(1.05); for(let i=0;i<6;i++){let m=cone(.25,.9,6);let a=i*Math.PI/3;m.position.set(Math.cos(a)*1.05,Math.sin(a)*1.05,0);m.rotation.z=a-Math.PI/2;} break; }
    case 9: { const core=ico(.7,2); torus(1.25,.11,Math.PI/2); torus(1.25,.11,0,Math.PI/2); break; }
    case 10: { const b=box(1.55,1.55,1.55); b.rotation.set(.45,.35,.2); torus(1.3,.08,Math.PI/2); break; }
    case 11: { for(let i=0;i<5;i++){let m=ico(.48,1);m.position.set((Math.random()-.5)*1.5,(Math.random()-.5)*1.5,(Math.random()-.5)*1.5);} break; }
    case 12: { cyl(.72,2.2,12); for(let y of [-.65,0,.65]) torus(.82,.07,Math.PI/2).position.y=y; break; }
    case 13: { cone(1.0,2.2,4); const t=torus(.85,.12,Math.PI/2);t.position.y=.55; break; }
    case 14: { sphere(.9); for(let i=0;i<3;i++){let t=torus(1.2,.08,0,i*Math.PI/3);t.rotation.x=.7;} break; }
    case 15: { const a=torus(1.05,.22,Math.PI/2); a.scale.set(1,1,.6); ico(.55,1); break; }
    case 16: { for(let i=0;i<4;i++){let m=box(.35,2.4,.35);m.rotation.y=i*Math.PI/4;m.rotation.z=i*.15;} sphere(.6); break; }
    case 17: { const o=torus(1.35,.18,Math.PI/2); o.scale.set(1,.7,1); const inner=torus(.8,.10,0,Math.PI/2); inner.scale.set(1,.7,1); break; }
    case 18: { crystal(1.35,1.8,1.35); const t=torus(1.15,.07,Math.PI/2);t.rotation.z=.4; break; }
    case 19: { for(let i=0;i<8;i++){let m=cone(.28,1.3,5);let a=i*Math.PI/4;m.position.set(Math.cos(a)*.85,Math.sin(a)*.85,0);m.rotation.z=a-Math.PI/2;} ico(.65,2); break; }
    case 20: { const b=box(1.8,.8,1.8); const lid=cone(1.35,.9,4);lid.position.y=.75; torus(1.0,.08,Math.PI/2); break; }
    case 21: { cyl(.65,1.9,8); for(let i=0;i<5;i++){let t=torus(1.0,.08,Math.PI/2);t.position.y=-.65+i*.32;} break; }
    case 22: { const base=ico(1.25,1); for(let i=0;i<4;i++){let t=torus(1.45,.07,0,i*Math.PI/4);t.rotation.x=.8;} break; }
    case 23: { sphere(1.1); for(let i=0;i<12;i++){let m=ico(.16,0);let a=i*Math.PI/6;m.position.set(Math.cos(a)*1.35,Math.sin(a)*1.35,Math.sin(a*2)*.35);} break; }
    case 24: { const t1=torus(1.25,.13,Math.PI/2); const t2=torus(1.25,.13,0,Math.PI/2); const t3=torus(1.25,.13,0,Math.PI/4); break; }
    case 25: { const c1=cyl(.9,.9,6); c1.rotation.x=.55; const c2=cyl(.9,.9,6); c2.rotation.x=-.55;c2.position.y=.9; sphere(.45).position.y=.25; break; }
    case 26: { const s=sphere(.7); for(let i=0;i<10;i++){let m=box(.18,1.9,.18);let a=i*Math.PI/5;m.position.set(Math.cos(a)*.85,Math.sin(a)*.85,0);m.rotation.z=a;} break; }
    case 27: { const o=ico(1.45,2); o.scale.set(1,.72,1); for(let i=0;i<3;i++){let t=torus(1.55,.06,0,i*Math.PI/3);t.rotation.x=.6;} break; }
    case 28: { const s=sphere(1.15); for(let i=0;i<3;i++){let r=torus(1.5,.09,Math.PI/2);r.rotation.z=i*Math.PI/3;} break; }
    case 29: { for(let i=0;i<7;i++){let c2=cone(.42,1.6,7);let a=i*Math.PI*2/7;c2.position.set(Math.cos(a)*.8,Math.sin(a)*.8,0);c2.rotation.z=a-Math.PI/2;} sphere(.55); break; }
    case 30: { const b=box(2.0,2.0,2.0);b.rotation.set(Math.PI/4,Math.PI/4,0); const t=torus(1.55,.1,Math.PI/2); break; }
    case 31: { sphere(1.15); const inner=ico(.55,2); inner.material=glowMat(); for(let i=0;i<4;i++){let t=torus(1.4,.055,Math.PI/2);t.rotation.z=i*Math.PI/4;} break; }
    case 32: { for(let i=0;i<6;i++){let m=ico(.55,1);let a=i*Math.PI/3;m.position.set(Math.cos(a)*1.0,Math.sin(a)*1.0,Math.sin(a)*.6);m.rotation.y=a;} torus(1.65,.08,Math.PI/2); break; }
    case 33: { const s=ico(1.1,2); for(let i=0;i<5;i++){let t=torus(1.5,.055,Math.PI/2);t.rotation.z=i*Math.PI/5;} break; }
    case 34: { const outer=torus(1.55,.22,Math.PI/2); const inner=torus(.75,.12,Math.PI/2); inner.material=glowMat(); for(let i=0;i<4;i++){let m=cone(.22,1.8,5);let a=i*Math.PI/2;m.position.set(Math.cos(a)*.95,Math.sin(a)*.95,0);m.rotation.z=a;} break; }
    case 35: { sphere(.95); for(let i=0;i<3;i++){let t=torus(1.35,.10,Math.PI/2);t.rotation.z=i*Math.PI/3;} ico(.52,2).material=glowMat(); break; }
    // Season 2: more mechanical / cosmic silhouettes
    case 36: { cyl(.8,2.1,8); const cap=torus(.9,.13,Math.PI/2);cap.position.y=1.05; break; }
    case 37: { const o=ico(1.35,1); for(let i=0;i<3;i++){let t=torus(1.55,.06,Math.PI/2);t.rotation.z=i*Math.PI/3;} break; }
    case 38: { sphere(1.0); for(let i=0;i<8;i++){let m=box(.22,.22,1.7);let a=i*Math.PI/4;m.rotation.z=a;m.position.set(Math.cos(a)*.55,Math.sin(a)*.55,0);} break; }
    case 39: { cone(1.15,2.3,3); const t=torus(1.1,.09,Math.PI/2);t.position.y=-.45; break; }
    case 40: { const b=box(1.8,1.1,1.8); for(let i=0;i<4;i++){let c2=cyl(.18,1.8,8);let a=i*Math.PI/2;c2.position.set(Math.cos(a)*.85,0,Math.sin(a)*.85);c2.rotation.z=Math.PI/2;} break; }
    case 41: { const o=torus(1.25,.18,Math.PI/2); const i=ico(.65,1); for(let z of [-.45,.45]){let t=torus(.55,.07,Math.PI/2);t.position.z=z;} break; }
    case 42: { crystal(1.1,1.8,1.1); const t=torus(1.4,.08,Math.PI/2);t.rotation.z=.7; break; }
    case 43: { const s=sphere(.75); for(let i=0;i<6;i++){let m=cone(.3,1.6,4);let a=i*Math.PI/3;m.position.set(Math.cos(a)*1.0,0,Math.sin(a)*1.0);m.rotation.z=Math.PI/2;} break; }
    case 44: { const o=ico(1.25,2); for(let i=0;i<4;i++){let t=torus(1.55,.07,Math.PI/2);t.rotation.z=i*Math.PI/4;} break; }
    case 45: { cyl(.95,1.0,16); const c2=cone(.75,1.9,16);c2.position.y=1.1; const t=torus(1.0,.08,Math.PI/2); break; }
    case 46: { const b=box(1.3,2.2,1.3); for(let i=0;i<4;i++){let t=torus(.95,.06,Math.PI/2);t.position.y=-.75+i*.5;} break; }
    case 47: { sphere(1.0); for(let i=0;i<12;i++){let m=ico(.13,0);let a=i*Math.PI/6;m.position.set(Math.cos(a)*1.4,Math.sin(a)*1.4,Math.cos(a*2)*.4);} break; }
    case 48: { const o=torus(1.4,.2,Math.PI/2); o.scale.set(1,.55,1); const c2=cone(.75,1.7,8);c2.position.y=.1; break; }
    case 49: { for(let i=0;i<3;i++){let m=ico(.8,1);m.position.set((i-1)*.95,Math.sin(i)*.35,0);} torus(1.7,.07,Math.PI/2); break; }
    case 50: { const b=box(1.8,1.8,1.8); b.rotation.set(.3,.6,.2); for(let i=0;i<4;i++){let m=box(.25,2.5,.25);m.rotation.y=i*Math.PI/4;} break; }
    case 51: { sphere(1.0); const a=torus(1.55,.09,Math.PI/2);a.rotation.z=.35; const b=torus(1.25,.07,0,Math.PI/2);b.rotation.x=.8; break; }
    case 52: { const base=cyl(.8,1.5,12); const top=cone(1.2,1.0,6);top.position.y=1.0; for(let i=0;i<3;i++){let t=torus(1.0,.06,Math.PI/2);t.position.y=-.45+i*.45;} break; }
    case 53: { const s=ico(1.15,2); for(let i=0;i<5;i++){let c2=cone(.25,1.8,5);let a=i*Math.PI*2/5;c2.position.set(Math.cos(a)*1.0,Math.sin(a)*1.0,0);c2.rotation.z=a-Math.PI/2;} break; }
    case 54: { const o=sphere(1.2); const i=sphere(.55);i.material=glowMat(); for(let i=0;i<2;i++){let t=torus(1.55,.1,Math.PI/2);t.rotation.z=i*Math.PI/2;} break; }
    case 55: { for(let i=0;i<9;i++){let m=box(.35,.35,1.7);let a=i*Math.PI*2/9;m.position.set(Math.cos(a)*.95,Math.sin(a)*.95,0);m.rotation.z=a;} ico(.6,1); break; }
    case 56: { const c1=ico(1.0,2); const c2=ico(.55,2);c2.material=glowMat();c2.position.z=.75; for(let i=0;i<4;i++){let t=torus(1.45,.055,Math.PI/2);t.rotation.z=i*Math.PI/4;} break; }
    case 57: { const o=torus(1.5,.25,Math.PI/2); const o2=torus(1.5,.08,0,Math.PI/2); sphere(.45); break; }
    case 58: { const s=ico(1.2,2); for(let i=0;i<6;i++){let m=cone(.2,2.0,6);let a=i*Math.PI/3;m.position.set(Math.cos(a)*.95,Math.sin(a)*.95,0);m.rotation.z=a-Math.PI/2;} break; }
    case 59: { const s=sphere(1.1); for(let i=0;i<5;i++){let t=torus(1.55,.08,Math.PI/2);t.rotation.z=i*Math.PI/5;} const gem=ico(.45,2);gem.material=glowMat();gem.position.y=.15; break; }
    case 60: { const o=ico(1.45,2);o.scale.set(1,.7,1); for(let i=0;i<3;i++){let t=torus(1.7,.12,Math.PI/2);t.rotation.z=i*Math.PI/3;} const core=ico(.55,2);core.material=glowMat(); break; }
  }

  // Level-based tilt/scale keeps the silhouettes distinct even on small screens.
  g.rotation.set((level%5)*.12,(level%7)*.19,(level%3)*.08);
  const scale = 1.0 + Math.min(level,35)*.008;
...
function buildObject(){
  ...
  sceneState.coreLight.intensity=14 + l*.15;
  sceneState.outer=model;
  sceneState.core=model.children.find(x=>x.material && x.material.emissiveIntensity>2) || model;
}
...
function loop(){
  ...
  if(outer){outer.rotation.x+=.004;outer.rotation.y+=.007;if(core){core.rotation.x-=.008;core.rotation.y-=.01;}}
