/* 지온냄새 강화하기 V17.49 · MOBILE PERFORMANCE OPTIMIZED */
...
function spawnDestroyShards(layer,color='#ef4444',count=28){
  ...
  const maxCount=reduced?6:Math.min(count,perfCount(count,10,7));
...
function playDestroyedCardSequence(actualCard,glow,finish,particleColor,commitVisual){
  ...
  addDestroyCracks(ghost,perfCount(15,9,6));
...
function renderCardDesign(art,l,d){
  ...
  const designParticleCount=Math.min(spec.particles,perfCount(spec.particles,7,4));
...
function playWarpAnimation(targetLevel,onShift,onComplete){
  ...
  const warpStreakCount=perfCount(24,14,9);
  overlay.innerHTML=`
...
function animateFinal54To55(status, afterFinish=null){
  ...
  const finalRayCount=perfCount(28,16,10),finalTickCount=perfCount(36,20,12);
...
function resultFxHoldStyle(label,color,kind='hold'){
  ...
  const holdRingCount=perfCount(5,4,3),holdTickCount=perfCount(24,14,9);
