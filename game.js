...
function enhance(){
  if(revivePromptOpen || enhancementLocked) return;
  ...
  animateResult(d.status, ()=>{
    if(d.status==="DESTROYED" && (d.reviveTickets||0)>0){
      showRevivePrompt();
    }else{
...
function showRevivePrompt(){
  ...
  document.getElementById("reviveNo").onclick=()=>{revivePromptOpen=false; overlay.remove(); enhancementLocked=false; render();};
  ...
  if(yes) yes.onclick=()=>{
    ...
    save(); render(); overlay.remove(); revivePromptOpen=false; enhancementLocked=true;
