...
function enhance(){
  if(revivePromptOpen || enhanceInputLocked) return;
  enhanceInputLocked=true;
  const button=document.getElementById("enhanceBtn");
  if(button) button.disabled=true;
  // 강화 버튼 입력 후 정확히 0.2초 뒤 결과 처리를 시작합니다.
  setTimeout(()=>{
    if(revivePromptOpen){ enhanceInputLocked=false; render(); return; }
    performEnhance();
  },200);
}
...
function render(){
  ...
  eb.disabled=(l>=max)||enhanceInputLocked;
  eb.textContent=enhanceInputLocked ? "✨ 결과 연출 중..." : "🔥 강화하기";
  sb.disabled=(l===0)||enhanceInputLocked;
  sb.textContent=`💰 판매하기`;
...
function animateResult(status, afterFinish=null){
  ...
  if(status==='SUCCESS'||status==='PITY_SUCCESS'){
    ...
    // 성공: 0.2초 정적 → 에너지 충전 → 대폭발 → 카드 점프 → 광택 스윕
    gsap.set(card,{scale:.76,filter:'brightness(.55) saturate(1.4)',y:18});
    gsap.set(glow,{scale:.12,opacity:.01});
    if(shine) gsap.set(shine,{x:'-120%',opacity:0});
    cardBurst(isPity?'#fde68a':'#22c55e',45,170);
    gsap.to(glow,{scale:1.35,opacity:isPity?.25:.18,duration:.2,ease:'power2.out'});
    gsap.to(card,{scale:.91,y:4,filter:'brightness(1.05) saturate(1.7)',duration:.22,ease:'power2.out'});
    gsap.delayedCall(.22,()=>{
      cardBurst(isPity?'#fef3c7':'#4ade80',95+(level()*3),300+level()*8);
      cardBurst('#ffffff',45+level(),220+level()*5);
      impactFlash(isPity?.55:.4); screenShake(isPity?.11:.065,.18);
      gsap.to(glow,{scale:2.25,opacity:isPity?.48:.38,duration:.32,ease:'power4.out'});
      gsap.to(card,{scale:1.08,filter:'brightness(1.85) saturate(2)',duration:.24,ease:'power4.out'});
    });
    gsap.delayedCall(.5,()=>{
      if(shine) gsap.to(shine,{x:'190%',opacity:1,duration:.48,ease:'power4.inOut'});
      cardBurst(isPity?'#fff7cc':'#86efac',120+level()*2,410+level()*9);
      impactFlash(isPity?.78:.62); screenShake(isPity?.18:.11,.2);
      gsap.to(card,{y:-30,scale:1.16,duration:.14,ease:'power4.out'});
    });
    gsap.delayedCall(.72,()=>{
      gsap.to(card,{y:0,scale:1,duration:.48,ease:'elastic.out(1,.42)',onComplete:finish});
      gsap.to(glow,{scale:1,opacity:.12,duration:.45,ease:'power2.out'});
    });
    return;
  }
  ...
  if(status==='CRITICAL'){
    ...
    // 크리티컬: 강제 정지 → 황금 코어 충전 → 초대형 폭발 → 화면 충격
    gsap.set(card,{scale:.68,filter:'brightness(.45) saturate(2.2)',y:24});
    gsap.set(glow,{scale:.05,opacity:0});
    cardBurst('#ffffff',70,230); cardBurst('#facc15',80,260); cardBurst('#fb923c',35,190);
    gsap.to(glow,{scale:1.7,opacity:.5,duration:.28,ease:'power4.out'});
    gsap.to(card,{scale:.84,filter:'brightness(1.4) saturate(2.8)',duration:.28,ease:'power3.out'});
    gsap.delayedCall(.28,()=>{
      impactFlash(1); screenShake(.25,.28);
      cardBurst('#fff',170,500); cardBurst('#facc15',150,460); cardBurst('#fb923c',80,330);
      gsap.to(glow,{scale:2.9,opacity:.72,duration:.28,ease:'expo.out'});
      gsap.to(card,{scale:1.14,filter:'brightness(2.5) saturate(3)',duration:.18,ease:'power4.out'});
      if(shine) gsap.to(shine,{x:'210%',opacity:1,duration:.4,ease:'power4.out'});
    });
    gsap.delayedCall(.55,()=>{
      impactFlash(.9); screenShake(.3,.3);
      cardBurst('#ffffff',190,560); cardBurst('#fde047',130,430);
      gsap.to(card,{y:-38,scale:1.23,duration:.13,ease:'power4.out'});
    });
    gsap.delayedCall(.72,()=>gsap.to(card,{y:0,scale:1,duration:.58,ease:'elastic.out(1,.3)',onComplete:finish}));
    return;
  }
  ...
  if(status==='FAILED'||status==='HOLD'){
    ...
    const shakeLevel=Math.max(1,Math.min(5.5,0.9+((state.seasonData[state.currentSeason].prev_level||0)*.095)));
    // 하락: 카드가 아래로 끌려가며 붉은 충격. 유지: 보라색 에너지가 얼어붙는 연출.
    if(failed){
      gsap.set(card,{scale:1.08,y:-8,filter:'brightness(1.15) saturate(1.4)'});
      gsap.set(glow,{scale:1.4,opacity:.16});
      cardBurst('#f87171',80+Math.floor(shakeLevel*18),250+shakeLevel*70);
      gsap.delayedCall(.16,()=>{
        impactFlash(.42); screenShake(.16,.22);
        cardBurst('#ef4444',85+Math.floor(shakeLevel*20),280+shakeLevel*75);
        gsap.to(card,{y:28,scale:.94,rotation:(Math.random()<.5?-1:1)*2.4,duration:.2,ease:'power4.in'});
        gsap.to(glow,{scale:.55,opacity:.35,duration:.24,ease:'power4.in'});
      });
      gsap.delayedCall(.38,()=>{
        screenShake(.12,.18); cardBurst('#ffffff',38,190);
        gsap.to(card,{x:-7,duration:.06,yoyo:true,repeat:5,ease:'none'});
      });
      gsap.delayedCall(.62,()=>{
        impactFlash(.28); cardBurst('#fca5a5',55,210);
        gsap.to(card,{x:0,y:0,rotation:0,scale:.97,filter:'brightness(.68) saturate(.7)',duration:.28,ease:'power3.out'});
      });
      gsap.delayedCall(.92,()=>gsap.to(card,{scale:1,filter:'brightness(1) saturate(1)',duration:.3,ease:'back.out(1.8)',onComplete:finish}));
    }else{
      gsap.set(card,{scale:.98,filter:'brightness(.85) saturate(.8)'});
      gsap.set(glow,{scale:.75,opacity:.05});
      cardBurst('#c4b5fd',55,180); cardBurst('#ffffff',18,120);
      gsap.to(glow,{scale:1.8,opacity:.3,duration:.3,ease:'power2.out'});
      gsap.to(card,{scale:1.045,duration:.3,ease:'sine.inOut',yoyo:true,repeat:3});
      gsap.delayedCall(.35,()=>{
        impactFlash(.22); screenShake(.06,.16);
        cardBurst('#a78bfa',65+Math.floor(shakeLevel*8),220+shakeLevel*45);
        gsap.to(glow,{scale:.55,opacity:.18,duration:.25,ease:'power3.in'});
      });
      gsap.delayedCall(.7,()=>gsap.to(card,{scale:.98,filter:'brightness(.9) saturate(.85)',duration:.2,ease:'power2.in'}));
      gsap.delayedCall(.94,()=>gsap.to(card,{scale:1,filter:'brightness(1) saturate(1)',duration:.32,ease:'back.out(1.6)',onComplete:finish}));
    }
    return;
  }
  ...
  if(status==='SHIELD_SAVED'){
    ...
    // 방지권: 파란 보호막이 카드 전체를 감싼 뒤 파괴 에너지를 튕겨냄
    gsap.set(card,{scale:.88,filter:'brightness(.8) saturate(1.35)',y:10});
    gsap.set(glow,{scale:.25,opacity:.02});
    cardBurst('#60a5fa',95,320); cardBurst('#dbeafe',35,180);
    impactFlash(.38);
    gsap.to(glow,{scale:2.2,opacity:.46,duration:.35,ease:'back.out(1.7)'});
    gsap.to(card,{scale:1.08,y:-10,filter:'brightness(1.55) saturate(1.7)',duration:.28,ease:'power3.out'});
    gsap.delayedCall(.28,()=>{
      screenShake(.14,.2); impactFlash(.5);
      cardBurst('#60a5fa',120,420); cardBurst('#ffffff',55,260);
      gsap.to(card,{scale:1.12,duration:.12,ease:'power4.out'});
    });
    gsap.delayedCall(.52,()=>{
      cardBurst('#93c5fd',90,360);
      gsap.to(glow,{scale:.7,opacity:.2,duration:.32,ease:'power3.in'});
      gsap.to(card,{scale:1,y:0,filter:'brightness(1) saturate(1)',duration:.42,ease:'elastic.out(1,.45)',onComplete:finish});
    });
    return;
  }
