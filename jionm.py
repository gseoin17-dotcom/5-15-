...
with left_col:
  ...
  @st.dialog("🛒 포인트 상점", width="large")
  def show_point_shop():
    st.markdown("""
    <style>
      [role="dialog"] { width: min(96vw, 1200px) !important; }
      [role="dialog"] > div { max-height: 92vh !important; }
      .panel-title { font-size:30px; font-weight:900; color:#fde68a; margin-bottom:4px; }
      .panel-sub { color:#94a3b8; font-size:14px; margin-bottom:18px; }
      .shop-card { padding:18px; border:1px solid rgba(255,255,255,.14); border-radius:18px; background:rgba(255,255,255,.055); margin-bottom:14px; }
      .shop-name { font-size:20px; font-weight:900; }
      .shop-info { color:#cbd5e1; font-size:13px; margin-top:5px; line-height:1.55; }
    </style>
    """, unsafe_allow_html=True)
    close_col, title_col = st.columns([1, 8])
    with close_col:
      if st.button("✕", key="close_shop", help="닫기"):
        st.session_state.open_panel = None
        st.rerun()
    with title_col:
      st.markdown("<div class='panel-title'>🛒 포인트 상점</div><div class='panel-sub'>방지권과 워프권을 돈 또는 포인트로 구매할 수 있습니다.</div>", unsafe_allow_html=True)

    m1, m2 = st.columns(2)
    with m1:
      st.metric("💰 보유 금액", format_gold(st.session_state.money))
    with m2:
      st.metric("⭐ 보유 포인트", f"{st.session_state.points:,}P")

    st.markdown("### 🛡️ 방지권")
    min_shield_level = 16 if st.session_state.is_rebirth else 20
    if st.session_state.is_rebirth:
      shield_money = int(SMELL_DB[True][st.session_state.level]["price"] / 5)
    else:
      shield_money = get_shield_cost(st.session_state.level, st.session_state.is_rebirth)
    shield_point = get_shield_point_cost(st.session_state.level, st.session_state.is_rebirth)
    st.markdown(f"<div class='shop-card'><div class='shop-name'>🛡️ 파괴 방지권</div><div class='shop-info'>보유: {st.session_state.shield} / 3개<br>구매 가능 단계: {min_shield_level}단계 이상<br>💰 {format_gold(shield_money)}　⭐ {shield_point:,}P</div></div>", unsafe_allow_html=True)
    if st.session_state.level < min_shield_level:
      st.warning(f"방지권은 {min_shield_level}단계 이상부터 구매할 수 있습니다.")
    elif st.session_state.shield >= 3:
      st.info("방지권은 최대 3개까지 보유할 수 있습니다.")
    else:
      c1, c2 = st.columns(2)
      with c1:
        if st.button(f"💰 돈으로 구매  {format_gold(shield_money)}", key="buy_shield_money", use_container_width=True):
          if st.session_state.money >= shield_money:
            st.session_state.money -= shield_money
            st.session_state.shield += 1
            save_current_season_state()
            st.success("🛡️ 방지권 구매 완료!")
            st.rerun()
          else:
            st.error("금액이 부족합니다.")
      with c2:
        if st.button(f"⭐ 포인트로 구매  {shield_point:,}P", key="buy_shield_point", use_container_width=True):
          if st.session_state.points >= shield_point:
            st.session_state.points -= shield_point
            st.session_state.points_spent_total += shield_point
            st.session_state.shield += 1
            save_current_season_state()
            st.success("🛡️ 방지권 구매 완료!")
            st.rerun()
    ...
    for w_level in warp_levels:
      point_price = get_warp_point_cost(w_level)
      money_price = get_warp_money_cost(w_level, st.session_state.is_rebirth)
      if st.session_state.is_rebirth:
        unlocked = st.session_state.unlocked_season2_warps.get(w_level, False) or st.session_state.max_level >= w_level
      else:
        unlocked = st.session_state.unlocked_warps.get(w_level, False) or st.session_state.max_level >= w_level
      disabled = (not unlocked) or (st.session_state.level >= w_level)
      st.markdown(f"<div class='shop-card'><div class='shop-name'>🚀 {w_level}단계 워프권</div><div class='shop-info'>💰 {format_gold(money_price)}　　⭐ {point_price:,}P　　{'구매 가능' if unlocked else '아직 잠김'}</div></div>", unsafe_allow_html=True)
      c1, c2 = st.columns(2)
      with c1:
        if st.button(f"💰 돈으로 구매  {format_gold(money_price)}", key=f"buy_warp_money_{st.session_state.is_rebirth}_{w_level}", use_container_width=True, disabled=disabled):
          if st.session_state.money >= money_price:
            st.session_state.money -= money_price
            st.session_state.warp_uses += 1
            st.session_state.prev_level = st.session_state.level
            st.session_state.level = w_level
            st.session_state.max_level = max(st.session_state.max_level, w_level)
            st.session_state.status = "SUCCESS"
            save_current_season_state(); check_achievements()
            st.success(f"🚀 {w_level}단계로 워프 성공!")
            st.rerun()
          else:
            st.error("금액이 부족합니다.")
      with c2:
        if st.button(f"⭐ 포인트로 구매  {point_price:,}P", key=f"buy_warp_point_{st.session_state.is_rebirth}_{w_level}", use_container_width=True, disabled=disabled):
          if st.session_state.points >= point_price:
            st.session_state.points -= point_price
            st.session_state.points_spent_total += point_price
            st.session_state.warp_uses += 1
            st.session_state.prev_level = st.session_state.level
            st.session_state.level = w_level
            st.session_state.max_level = max(st.session_state.max_level, w_level)
  ...
  def show_tears():
    st.markdown("""
    <style>[role="dialog"] { width: min(96vw, 1200px) !important; } [role="dialog"] > div { max-height: 92vh !important; }</style>
    """, unsafe_allow_html=True)
    c, h = st.columns([1, 8])
    with c:
      if st.button("✕", key="close_tears", help="닫기"):
        st.session_state.open_panel = None
        st.rerun()
    with h:
      st.markdown("<div class='panel-title'>💧 눈물</div>", unsafe_allow_html=True)
    max_lvl = 25 if st.session_state.is_rebirth else 35
    limit_lvl = 18 if st.session_state.is_rebirth else 32
    st.metric("💧 현재 눈물", f"{st.session_state.tears} / 60개")
    if st.session_state.level >= limit_lvl:
      st.warning("⚠️ 고단계부터는 눈물을 사용할 수 없습니다!")
    else:
      st.info("눈물 20개를 사용하면 100% 확률로 1~3단계 상승합니다.")
    can_use_tears = st.session_state.level < limit_lvl and st.session_state.tears >= 20
    if st.button("💧 눈물 기적 가동", key="tears_action", use_container_width=True, disabled=not can_use_tears):
      ...
      st.rerun()
    if st.session_state.level < limit_lvl and st.session_state.tears < 20:
      st.caption("눈물이 20개 이상 있어야 사용할 수 있습니다.")
  ...
  def show_achievements():
    ...
    c, h = st.columns([1, 8])
    with c:
      if st.button("✕", key="close_ach", help="닫기"):
        st.session_state.open_panel = None
        st.rerun()
    with h:
      st.markdown("<div class='panel-title'>🏆 업적</div>", unsafe_allow_html=True)
    ...
    for i, (key, info) in enumerate(achievement_items):
      ...
      bg = f"linear-gradient(135deg,{deep},{accent2}55,#020617)" if done else "linear-gradient(135deg,rgba(15,23,42,.96),rgba(2,6,23,.99))"
      border = accent if done else "rgba(148,163,184,.22)"
      with ach_cols[i % 3]:
        st.markdown(f"<div class='title-design {title_style}' style='background:{bg};border:1px solid {border};box-shadow:0 0 22px {accent}25;border-radius:16px;padding:15px;margin:0 0 12px 0;min-height:125px;'><div style='font-size:10px;letter-spacing:1.5px;color:{accent if done else '#64748b'}'>{'UNLOCKED' if done else 'LOCKED'}</div><div style='font-size:16px;font-weight:900;margin-top:5px'>{icon} {info['name']}</div><div style='font-size:12px;color:#cbd5e1;margin-top:7px'>{info['desc']}</div><div style='font-size:11px;color:{accent};margin-top:9px;font-weight:800'>🏷️ {info['title']}</div><div style='font-size:10px;color:#fde68a;margin-top:2px'>💰 {format_gold(info['reward'])}</div></div>", unsafe_allow_html=True)
    options = [TITLE_DEFAULT] + st.session_state.unlocked_titles
    if st.session_state.selected_title not in options:
      st.session_state.selected_title = TITLE_DEFAULT
    st.session_state.selected_title = st.selectbox("현재 칭호", options, index=options.index(st.session_state.selected_title), key="achievement_title_select")

  nav_shop, nav_tears, nav_ach = st.columns(3)
  with nav_shop:
    if st.button("🛒 포인트 상점", key="open_shop", use_container_width=True):
      st.session_state.open_panel = "shop"
      st.rerun()
  with nav_tears:
    if st.button("💧 눈물", key="open_tears", use_container_width=True):
      st.session_state.open_panel = "tears"
      st.rerun()
  with nav_ach:
    if st.button("🏆 업적", key="open_ach", use_container_width=True):
      st.session_state.open_panel = "ach"
      st.rerun()

  if st.session_state.open_panel == "shop":
    show_point_shop()
  elif st.session_state.open_panel == "tears":
    show_tears()
  elif st.session_state.open_panel == "ach":
    show_achievements()
